...
export default function AdminCutoffs() {
  ...
  const exportHistoryCSV = () => {
    ...
    const lines = history.map(c => {
      const fecha = dayjs(c.createdAt).format('YYYY-MM-DD HH:mm');
      const monto = c.amount;
      const desde = c.from ? dayjs(c.from).format('YYYY-MM-DD HH:mm') : '';
      const hasta = c.to ? dayjs(c.to).format('YYYY-MM-DD HH:mm') : '';
    ...
    a.href = url;
    a.download = `cortes_${dayjs().format('YYYYMMDD_HHmm')}.csv`;
    a.click();
        ...
        <div className="card-body d-flex align-items-center" style={{ gap: 16 }}>
          ...
          <div className="text-muted">
            {pending.from && pending.to
              ? <>Rango: <strong>{dayjs(pending.from).format('YYYY-MM-DD HH:mm')}</strong> → <strong>{dayjs(pending.to).format('YYYY-MM-DD HH:mm')}</strong></>
              : 'Sin rango pendiente'}
          </div>
          <button
          ...
          <tbody>
            {history.map(c => (
              <tr key={c._id}>
                ...
                <td>{c.from ? dayjs(c.from).format('YYYY-MM-DD HH:mm') : '—'}</td>
                <td>{c.to ? dayjs(c.to).format('YYYY-MM-DD HH:mm') : '—'}</td>
              </tr>
            ))}
            {!history.length && (
