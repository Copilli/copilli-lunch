  ...
  const fetchPayments = async () => {
    ...
    try {
      const { data } = await axios.get(`${API}/payments`, {
        params: { from, to, studentId: studentId || undefined },
        headers
      });
  ...
  const resendPending = async () => {
    try {
      const { data } = await axios.post(`${API}/payments/resend-mails`, {
        from, to, studentId: studentId || undefined
      }, { headers });
    ...
    <div className="container py-3">
      ...
      <div className="row g-2 mb-3">
        ...
        <div className="col-12 col-md-3 d-flex align-items-end">
          <button className="btn btn-primary w-100" onClick={fetchPayments} disabled={loading}>
      ...
      <div className="d-flex align-items-center mb-2">
        ...
        <div className="ms-auto d-flex align-items-center gap-2">
          ...
          <select className="form-select form-select-sm" style={{ width: 160 }} value={groupBy} onChange={e => setGroupBy(e.target.value)}>
          ...
          <tbody>
            {summary.map((r, idx) => (
              <tr key={idx}>
                <td>{groupBy === 'student'
                  ? r.studentId
                  : dayjs(r.date).format('YYYY-MM-DD')}</td>
      ...
      <div className="table-responsive">
        ...
        <tbody>
            {rows.map(r => (
              <tr key={r._id}>
                <td style={{whiteSpace:'nowrap'}}>{dayjs(r.date).format('YYYY-MM-DD HH:mm')}</td>
